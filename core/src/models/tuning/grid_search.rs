...
pub struct GridSearchConfig {
    // each element has a configuration and its metrics as a tuple
    pub configs: Vec<(HashMap<String, Value>, Option<Vec<Metric>>)>,
}
...
impl GridSearchConfig {
    ...
    pub fn build_configurations(&mut self, tunable_params: &HashMap<String, Value>, gt_config: &HashMap<String, Value>) {
        ...
        for (key, param) in &tune_params {
            match param {
                Value::Array(_) => {
                    ...
                    //total_configs *= arr_temp[2].as_u64().unwrap();
                },
                _ => {
        ...
        loop {
            ...
            for i in 0..total_configs {
                ...
                self.configs.push((initial_params.clone(), None));
            }
        ...
        for config in self.configs.clone() {
            println!("{:?} {:?} {:?}", config.0.get("acc_cov"), config.0.get("min_radius"), config.0.get("scan_resolution"));
        }
    ...
    fn unpack_f64_array(&self, arr: &Vec<Value>) -> Vec<Value> {
        ...
        for i in 0..arr[2].as_u64().unwrap() {
